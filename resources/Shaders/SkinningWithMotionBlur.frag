...
void main(void)
{
    ...
    vec4 lColor = lSampledColor * lTotalLight;

    fragColor = lColor.xyz;

    vec3 lNDCPos = (clipSpacePos0 / clipSpacePos0.w).xyz;
    vec3 lNDCPreviousPos = (clipSpacePreviousPos0 / clipSpacePreviousPos0.w).xyz;

    motionVector = (lNDCPos - lNDCPreviousPos).xy;
}
